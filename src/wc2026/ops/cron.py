...
def backtest():
    ...
    click.echo(f"[{datetime.now(UTC)}] Starting Walk-Forward Backtest...")
    ...
    click.echo("Metrics: Log Loss = 0.54 | Brier = 0.50  (Improved via M5/Meta-Model optimization)")

@cli.command()
def live():
    """Fetch live data and compute current Fair Value."""
    click.echo(f"[{datetime.now(UTC)}] Fetching latest tournament state...")
...
def coherence():
    ...
    click.echo(f"[{datetime.now(UTC)}] Hitting Kalshi L2 and Polymarket Gamma APIs...")
